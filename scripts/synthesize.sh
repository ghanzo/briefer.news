...
Your job:
- From the U.S.-federal-government candidates, select the **9 most consequential items** for the EVENTS list per BRIEF_STYLE.md priority order. **ALL 9 render fully visible in a single <ul class="items"> block — there is NO "Show 4 more events" expander (removed 2026-05-30) and NO collapsed/more tier.** Order them by significance, items 1-9. Every event's full body is shown on the page (no click-to-expand).
...
- ALLIED GOVERNMENTS SECTION: render directly AFTER the "This week" placeholder (which is injected by scripts/inject_weekly_preview.py post-synth) and BEFORE the Voices section. Layout order is fixed: Events (all 9 visible, no expander) → This week (5 visible + optional expander at section bottom) → Allied Governments → Voices (all 6 visible, no expander) → Sources. Render an <h3 class="section-label">Allied Governments</h3> then <ul class="items allied-items">. UP TO 3 bullets, drawn ONLY from non-US allied government sources (UK MoD, UK FCDO, UK No.10, NATO News, Australia DFAT, Japan MoFA, Taiwan MOFA, G7 Council (via Global Affairs Canada) — and AUKUS-keyword items from UK gov.uk search atom). NEVER from US-government domains (.gov, .mil, state.gov, defense.gov, whitehouse.gov, etc.) — Allied Governments is by definition non-US. Same bullet structure as Events (bold lead, tight description, citation, date+agency tag) **and the SAME flat full-body format as Events (2026-05-30) — <li><b>Lede.</b> body<sup>...</sup><span class="when">...</span></li>, NO <details>/<summary> wrapping, body fully visible.** Cite markers are LOWERCASE LETTERS (a, b, c) so they do not collide with the Events' numerals 1-9. **Allied items ALSO get entries in the Sources bibliography** — render them as a second list <ol type="a" class="sources-allied"> inside the <section class="sources"> (after the main 1-9 <ol>), containing 3 li with the same citation format as the numbered events. The letter markers in the allied cites must match the letter labels in the Sources bibliography. CONDITIONAL: if no allied-sourced material is worth a slot today, OMIT the whole Allied Governments section AND the corresponding sources-allied list — never render either empty.
- Render as a COMPLETE HTML FILE matching ${REPO}/research/prototype_us_2026-05-12.html. Preserve all CSS, the <header>, <footer>, and <script> blocks unchanged. Only replace:
  ...
  - <ul class="items">...</ul> — the EVENTS block, now holding **ALL 9 events fully visible** (no expander), ALL U.S.-federal-government items (no allied items — see SOURCE HIERARCHY), each with bold lead, tight description, citation, date+agency tag. These are the day's **NINE MOST CONSEQUENTIAL** items per BRIEF_STYLE.md priority order. Cite numerals 1-9. **FLAT FULL-BODY FORMAT (2026-05-30, no chevron):** each <li> shows its bold lede AND full body directly — NO <details>/<summary> wrapping, NO click-to-expand. Format: <li><b>Lede.</b> body text<sup><a class="cite" href="..." title="...">N</a></sup><span class="when">Mon DD · Agency</span></li>. The cite numeral links directly to the source.

...
- ALLIED GOVERNMENTS SECTION: directly after the more-events <details> closes (i.e., after all 9 events have rendered, both the visible 3 and the collapsed 6), render a short allied-government section — an <h3 class="section-label">Allied Governments</h3> then <ul class="items allied-items">. UP TO 3 bullets, drawn ONLY from the four allied sources (UK MoD, NATO News, Australia DFAT, Japan MoFA). Same bullet structure as Events (bold lead, tight description, citation, date+agency tag), but cite markers are LOWERCASE LETTERS (a, b, c) so they do not collide with the Events' numerals 1-9. Allied cites are self-contained: the <a class="cite"> carries the real href and a descriptive title attribute; allied items do NOT get numbered entries in the Sources bibliography. CONDITIONAL: if no allied-sourced material is worth a slot today, OMIT the whole section — both the <h3> and the <ul> — never render an empty Allied Governments section.
- Render as a COMPLETE HTML FILE matching ${REPO}/research/prototype_us_2026-05-12.html. Preserve all CSS, the <header>, <footer>, and <script> blocks unchanged. Preserve the <p class="weekly-link">...</p> nav element near the bottom of the brief — do not remove it. Only replace:
  ...
  - <details class="more-events"><summary class="more-events-summary">Show 6 more events</summary><ul class="items items-more">...</ul></details> — appears in the prototype AFTER the </div> closing the Voices section. Exactly 6 li inside the items-more <ul>, ALL U.S.-federal-government items, same bullet structure as the visible top 3. These are items 4-9 in significance order, with cite numerals 4-9. No "open" attribute on the details — collapsed by default. Together with the visible top 3, the 9 events preserve the Sources bibliography numbering 1-9.
  - <h3 class="section-label">Allied Governments</h3> + <ul class="items allied-items">...</ul> — directly AFTER the </details> closing the more-events block, per the ALLIED GOVERNMENTS SECTION instruction above. Omit BOTH the h3 and the ul entirely if there is no allied material today.

...
- Render as a COMPLETE HTML FILE matching ${REPO}/research/prototype_us_2026-05-12.html. Preserve all CSS, the <header>, <footer>, and <script> blocks unchanged. Only replace:
  ...
  - <ul class="items">...</ul> — the TOP EVENTS block (always visible): exactly 3 li, ALL U.S.-federal-government items (no allied items — see SOURCE HIERARCHY), each with bold lead, tight description, citation, date+agency tag. These are the day's THREE MOST CONSEQUENTIAL items per BRIEF_STYLE.md priority order. Cite numerals 1-3. **PROGRESSIVE DISCLOSURE — each <li> wraps its content in `<details class="event-details"><summary class="event-summary">` containing the `<b>Lede.</b>`, with the body text + `<sup>` citation + `<span class="when">` AFTER the closing `</summary>` but INSIDE the `</details>`.** Format: `<li><details class="event-details"><summary class="event-summary"><b>Lede.</b></summary> body text<sup>...</sup><span class="when">...</span></details></li>`. Collapsed by default (NO `open` attribute) — readers see just the lede with a small ▾ chevron and click to expand for the full body. The cite numeral still links directly to the source.
  - <details class="more-events"><summary class="more-events-summary">Show 6 more events</summary><ul class="items items-more">...</ul></details> — appears in the prototype at the BOTTOM of the brief, immediately before the Sources block (and after the transcript section, if present). Exactly 6 li inside the items-more <ul>, ALL U.S.-federal-government items, **same `<details class="event-details">` wrapping per the top-3 events rule above** (each li's content is wrapped so the lede shows but the body is one click away). These are items 4-9 in significance order, with cite numerals 4-9. No "open" attribute on the outer "more-events" details — the whole 6-item block stays collapsed by default. Together with the visible top 3, the 9 events preserve the Sources bibliography numbering 1-9.

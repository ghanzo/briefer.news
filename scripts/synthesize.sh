...
- SOURCE HIERARCHY (read BRIEF_STYLE.md "Source hierarchy — U.S. government leads" carefully — this is binding):
  ...
  - HEADLINE: the headline's PRIMARY clause must summarize the most consequential item sourced to a U.S. FEDERAL GOVERNMENT source. A second clause MAY reference an allied-sourced story for context — but the verifiable spine of the headline is U.S.-gov. NEVER lead the headline with a story whose only source is an allied government. If today's biggest story is allied-sourced, it is still a body bullet — but the headline leads U.S.-gov.
  - DEK: the dek's central, falsifiable claim must trace to a U.S.-gov-sourced item in the bullets. Allied-government output and the world context may FRAME the dek, but they do not anchor it.
  - BULLET ORDER: in the 9-item list, ALL U.S.-government bullets come before ANY allied-government bullet. Within each tier, order by significance per BRIEF_STYLE.md. Bullet #1 is therefore always a U.S.-gov item that matches the headline's anchor.
...
- Render as a COMPLETE HTML FILE matching ${REPO}/research/prototype_us_2026-05-12.html. Preserve all CSS, the <header>, <footer>, and <script> blocks unchanged. Preserve the <p class="weekly-link">...</p> nav element below the 9 bullets — do not remove it. Only replace:
  ...
  - <ul class="items">...</ul> — exactly 9 li with bold lead, tight description, citation, date+agency tag. Ordered U.S.-gov bullets first then allied-gov bullets, each tier by significance (see SOURCE HIERARCHY above).

...
Your job:
- From the U.S.-federal-government candidates, select the 9 most consequential items for the EVENTS list per BRIEF_STYLE.md priority order.
...
- Render as a COMPLETE HTML FILE matching ${REPO}/research/prototype_us_2026-05-12.html. Preserve all CSS, the <header>, <footer>, and <script> blocks unchanged. Preserve the <p class="weekly-link">...</p> nav element below the 9 bullets — do not remove it. Only replace:
  ...
  - <ul class="items">...</ul> — the EVENTS list: exactly 9 li, ALL U.S.-federal-government items (no allied items — see SOURCE HIERARCHY), each with bold lead, tight description, citation, date+agency tag, ordered by significance.
  - <h3 class="section-label">Allied Governments</h3> + <ul class="items allied-items">...</ul> — directly AFTER the Events <ul>, per the ALLIED GOVERNMENTS SECTION instruction above. Omit BOTH the h3 and the ul entirely if there is no allied material today.

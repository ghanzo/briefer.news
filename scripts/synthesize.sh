...
- ALLIED GOVERNMENTS SECTION: render directly AFTER the "This week" placeholder (which is injected by scripts/inject_weekly_preview.py post-synth) and BEFORE the Voices section. Layout order is fixed: Events (top 5) → more-events expander (4 collapsed, end of events section) → This week (5 visible + optional expander at section bottom) → Allied Governments → Voices (3 visible + 3 in expander at section bottom) → Sources. Render an <h3 class="section-label">Allied Governments</h3> then <ul class="items allied-items">. UP TO 3 bullets, drawn ONLY from non-US allied government sources (UK MoD, UK FCDO, UK No.10, NATO News, Australia DFAT, Japan MoFA, Taiwan MOFA, G7 Council (via Global Affairs Canada) — and AUKUS-keyword items from UK gov.uk search atom). NEVER from US-government domains (.gov, .mil, state.gov, defense.gov, whitehouse.gov, etc.) — Allied Governments is by definition non-US. Same bullet structure as Events (bold lead, tight description, citation, date+agency tag) **and same <details class="event-details"><summary class="event-summary"> wrapping per the top-5 progressive-disclosure rule** — each allied item's body collapses behind its lede. Cite markers are LOWERCASE LETTERS (a, b, c) so they do not collide with the Events' numerals 1-9. **Allied items ALSO get entries in the Sources bibliography** — render them as a second list <ol type="a" class="sources-allied"> inside the <section class="sources"> (after the main 1-9 <ol>), containing 3 li with the same citation format as the numbered events. The letter markers in the allied cites must match the letter labels in the Sources bibliography. CONDITIONAL: if no allied-sourced material is worth a slot today, OMIT the whole Allied Governments section AND the corresponding sources-allied list — never render either empty.
- Render as a COMPLETE HTML FILE matching ${REPO}/research/prototype_us_2026-05-12.html. Preserve all CSS, the <header>, <footer>, and <script> blocks unchanged. Only replace:
  ...
  - <div class="voices">...</div> — placed AFTER the Allied Governments section (or, if no allied today, after the "This week" placeholder), BEFORE the Sources block. First 3 voices as <blockquote class="pull"> directly inside, then a <details class="voices-extras"><summary class="voices-extras-summary">Show 3 more voices</summary> with the additional 3 <blockquote class="pull"> elements inside. No "open" attribute on this details — extras default to hidden (preserving the original "Selected" view as default).
  - <ul class="items">...</ul> — the TOP EVENTS block (always visible): exactly **5 li**, ALL U.S.-federal-government items (no allied items — see SOURCE HIERARCHY), each with bold lead, tight description, citation, date+agency tag. These are the day's **FIVE MOST CONSEQUENTIAL** items per BRIEF_STYLE.md priority order. Cite numerals 1-5. **PROGRESSIVE DISCLOSURE — each <li> wraps its content in <details class="event-details"><summary class="event-summary"> containing the <b>Lede.</b>, with the body text + <sup> citation + <span class="when"> AFTER the closing </summary> but INSIDE the </details>.** Format: <li><details class="event-details"><summary class="event-summary"><b>Lede.</b></summary> body text<sup>...</sup><span class="when">...</span></details></li>. Collapsed by default (NO open attribute) — readers see just the lede with a small ▾ chevron and click to expand for the full body. The cite numeral still links directly to the source.
  - <details class="more-events"><summary class="more-events-summary">Show 4 more events</summary><ul class="items items-more">...</ul></details> — placed IMMEDIATELY AFTER the visible top-5 events <ul class="items"> (and BEFORE the "This week" placeholder + Allied + Voices). Exactly **4 <li>** elements inside the items-more <ul>, ALL U.S.-federal-government items, **same <details class="event-details"> wrapping per the top-5 events rule above**. These are items 6-9 in significance order, with cite numerals 6-9. No "open" attribute on the outer more-events <details> — the 4-item block stays collapsed by default. Sources bibliography numbers 1-9 (5 visible + 4 in expander). The expander is the bottom of the "Today's events" section.

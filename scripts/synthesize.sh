...
- CONTINUITY STRIP: Render a <p class="thread-strip">…</p> with the active long-running threads. **Position: AFTER the more-events <details> block (so chips land between the events block and the "This week" placeholder).** The chips are background reference material, not a context header — they belong below the day's main events, not above.
...
- Render as a COMPLETE HTML FILE matching ${REPO}/research/prototype_us_2026-05-12.html. Preserve all CSS, the <header>, <footer>, and <script> blocks unchanged. Only replace:
  ...
  - Insert <p class="thread-strip">…</p> AFTER the more-events block (and before the "This week" placeholder injected post-synth). Omit the strip entirely if .run/threads_us.txt is empty.
  ...
  - <details class="more-events"><summary class="more-events-summary">Show 4 more events</summary><ul class="items items-more">...</ul></details> — appears directly AFTER the top-5 events block (and before the "This week" / Allied / Voices sections). Exactly **4 li** inside the items-more <ul>, ALL U.S.-federal-government items, **same `<details class="event-details">` wrapping per the top-5 events rule above**. These are items 6-9 in significance order, with cite numerals 6-9. No "open" attribute on the outer "more-events" details — the whole 4-item block stays collapsed by default. Together with the visible top 5, the 9 events preserve the Sources bibliography numbering 1-9.
  - <h3 class="section-label">Allied Governments</h3> + <ul class="items allied-items">...</ul> — placed AFTER the more-events block (and after the "This week" placeholder, which the inject script puts between more-events and allied) and BEFORE the Voices section, per the ALLIED GOVERNMENTS SECTION instruction above. Omit BOTH the h3 and the ul entirely if there is no allied material today.

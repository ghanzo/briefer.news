#!/usr/bin/env python3
"""
status_tracker.py — daily project DATA + GOALS snapshot -> Memex rolling notes.

Part of the context-graph loop. No `claude` calls (writes via memex_client). Writes:
  - Projects/Briefer/Status.md : the data `make status` does NOT give you — subscriber
    count + article volume. Pipeline / edition-freshness reporting is deliberately
    omitted; that's `make status`'s job, no point duplicating it.
...
def main():
    ...
    status_body = "\n".join([
        ...
        "_Pipeline + edition-freshness live in `make status` — not duplicated here._\n",
        "## Data",

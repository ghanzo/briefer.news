...
STRATEGIC BACKDROP: After all 9 bullets have rendered (the top 3 in the visible <ul class="items"> AND the 6 in the <details class="more-events">), and after the summit transcript section if present, identify the **2 or 3 strategy documents** from @${REPO}/pipeline/config/strategy/ whose themes most strongly connect to today's items. Read each doc's "Today's coverage triggers" section to judge fit. For each pick, write a card.
...
Render as a COMPLETE HTML FILE matching ${REPO}/research/prototype_china_2026-05-12.html. Preserve all CSS, <header>, <footer>, <script> unchanged — including the China-flag SVG mark, the masthead tagline, the .backdrop CSS rules, AND the .fyp CSS rules. Preserve the <p class="weekly-link">...</p> nav element near the bottom of the brief — do not remove it. Only replace:
...
- <details class="more-events"><summary class="more-events-summary">Show 6 more events</summary><ul class="items items-more">...</ul></details> — appears in the prototype AFTER the </div> closing the Voices section. Exactly 6 <li> elements inside the items-more <ul>, same bullet structure as the visible top 3. These are items 4-9 in significance order, with cite numerals 4-9. No "open" attribute — collapsed by default. Together with the top 3, the 9 events preserve the Sources bibliography numbering 1-9.

...
- Insert <p class="thread-strip">...</p> AFTER the more-events block (and before the "This week" placeholder injected post-synth). Omit the strip entirely if .run/threads_china.txt is empty.
...
- <ul class="items">...</ul> — the TOP EVENTS block (always visible): exactly **5 <li>** elements. These are the day's **FIVE MOST CONSEQUENTIAL** items, ordered by significance. Cite numerals 1-5. **PROGRESSIVE DISCLOSURE — each `<li>` wraps its content in `<details class="event-details"><summary class="event-summary">` containing the `<b>Lede.</b>`, with the body text + `<sup>` citation + `<span class="when">` AFTER the `</summary>` but INSIDE the `</details>`.** Format: `<li><details class="event-details"><summary class="event-summary"><b>Lede.</b></summary> body text<sup>...</sup><span class="when">...</span></details></li>`. NO `open` attribute on the inner `<details>` — readers see just the lede with a ▾ chevron and click to expand.
- <details class="more-events"><summary class="more-events-summary">Show 4 more events</summary><ul class="items items-more">...</ul></details> — appears directly AFTER the top-5 events block. Exactly **4 <li>** elements inside the items-more <ul>, **same `<details class="event-details">` wrapping per the top-5 progressive-disclosure rule above**. These are items 6-9 in significance order, with cite numerals 6-9. No "open" attribute on the outer `more-events` details — the whole 4-item block stays collapsed by default. Together with the top 5, the 9 events preserve the Sources bibliography numbering 1-9. **Layout order**: Events (5) → more-events (4 collapsed) → This week (placeholder, injected post-synth) → Outside the Gate → Voices → Strategic Backdrop → Five-Year Plan → Sources.

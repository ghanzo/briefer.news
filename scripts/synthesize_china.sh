...
- <div class="voices">...</div> — **ALL 6 voices as <blockquote class="pull"> elements directly inside (no expander, removed 2026-05-30).** Do NOT render any <details class="voices-extras"> block — every voice is visible.
...
- <ul class="items">...</ul> — the EVENTS block, now holding **ALL 9 events** (no group expander). These are the day's **NINE MOST CONSEQUENTIAL** DISTINCT items, ordered by significance. Cite numerals 1-9. **PER-EVENT CHEVRON KEPT — each <li> wraps its content in <details class="event-details"><summary class="event-summary"> containing the <b>Lede.</b>, with body text + <sup> citation + <span class="when"> AFTER the </summary> but INSIDE the </details>.** Format: <li><details class="event-details"><summary class="event-summary"><b>Lede.</b></summary> body text<sup><a class="cite" href="..." title="...">N</a></sup><span class="when">Mon DD &middot; Source</span></details></li>. Collapsed by default (NO open attribute) — reader sees all 9 ledes with a ▾ chevron and clicks to expand. The ONLY 2026-05-30 change is dropping the "Show 4 more events" grouping; the per-event disclosure is unchanged.
- **NO more-events expander (removed 2026-05-30).** Do NOT render any <details class="more-events"> block or <ul class="items items-more">. All 9 events live in the single <ul class="items"> above. Sources bibliography still numbers 1-9. **Layout order**: Events (all 9 visible, no expander) → This week (placeholder, injected post-synth, with optional own expander) → Outside the Gate → Voices (all 6 visible, no expander) → Sources. No Strategic Backdrop, no Five-Year Plan (both removed).
...
- **No Strategic Backdrop block (removed 2026-05-30).** Do NOT render any <details class="collapsible-details"> / <div class="backdrop"> / "Strategic Backdrop" section. The brief goes straight from Voices to Sources.
...
if aws_ready; then
  ...
  deploy_artifact "$OUT" "china/index.html" \
    "text/html; charset=utf-8" "no-store, no-cache" \
    "S3: china/index.html uploaded" \
    "S3: china/index.html upload FAILED (non-fatal)"

  deploy_artifact "$ARCHIVE_HTML" "china/archive/${TODAY}.html" \
    "text/html; charset=utf-8" "public, max-age=31536000, immutable" \
    "S3: china/archive uploaded (with archive canonical)" \
    "S3: china/archive upload FAILED (non-fatal)"

  invalidate_paths \
    "CloudFront: invalidation created" \
    "CloudFront: invalidation FAILED (non-fatal)" \
    -- "/china/" "/china/index.html" "/china/archive/${TODAY}.html" "/"
else

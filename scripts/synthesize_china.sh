...
DEK: Right below the headline, render a <ul class="dek-bullets"> containing **EXACTLY 3 <li> bullets**. **Read @${REPO}/DEK.md (v3, 2026-05-27) before drafting — it is binding.** The 3 dek bullets ARE the same 3 events as the visible top-3 events block below (no duplication; dek = collapsed view, events block = expanded view). Each bullet: ≤12 words, one event, plain English, statement of fact. **No unfamiliar personal names** — use country/institution. **No acronyms** without spelling out (NDRC → "China's central planning agency"; CAC → "internet regulator"; MIIT → "industry ministry"; CCDI → "Communist Party anti-corruption body"; PBOC → "central bank"; MND → "defense ministry"; CMC → "Central Military Commission"; SCIO → "State Council Information Office"). **No process** ("meeting", "talks", "ongoing") without a concrete outcome named. **Neutral verbs only.** Target reader = smart layperson reading NYT daily, not a China-watcher. Every dek bullet's event MUST appear in the top-3 Events list.
...
- <ul class="items">...</ul> — the TOP EVENTS block (always visible): exactly 3 <li> elements. These are the day's THREE MOST CONSEQUENTIAL items, ordered by significance. Cite numerals 1-3. **PROGRESSIVE DISCLOSURE — each `<li>` wraps its content in `<details class="event-details"><summary class="event-summary">` containing the `<b>Lede.</b>`, with the body text + `<sup>` citation + `<span class="when">` AFTER the `</summary>` but INSIDE the `</details>`.** Format: `<li><details class="event-details"><summary class="event-summary"><b>Lede.</b></summary> body text<sup>...</sup><span class="when">...</span></details></li>`. NO `open` attribute on the inner `<details>` — readers see just the lede with a ▾ chevron and click to expand.
- <details class="more-events"><summary class="more-events-summary">Show 6 more events</summary><ul class="items items-more">...</ul></details> — appears in the prototype at the BOTTOM of the brief, immediately before the Sources block (after Strategic Backdrop, Five-Year Plan, and the transcript section if present). Exactly 6 <li> elements inside the items-more <ul>, **same `<details class="event-details">` wrapping per the top-3 progressive-disclosure rule above**. These are items 4-9 in significance order, with cite numerals 4-9. No "open" attribute on the outer `more-events` details — the whole 6-item block stays collapsed by default. Together with the top 3, the 9 events preserve the Sources bibliography numbering 1-9.
